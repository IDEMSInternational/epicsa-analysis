...
for (s in seq_along(stations)){
  ...
  predict_df_1$pr_rainday1_lag <- predict_df_1$rainday_lag
  predict_df_1$rainday_lag <- as.factor(predict_df_1$rainday_lag)
  predict_df_1$pr_rainday1_lag <- as.factor(predict_df_1$pr_rainday1_lag)
  ...
  for(i in seq_along(products)) {
   ...
   predict_df_m$product <- products[[i]] 
  predict_df_m <- predict_df_m %>%
    mutate(product1 = ifelse(startsWith(variable, "station_"), "station1", "product1"))
...
for(s in seq_along(stations)) {
  ...
  ggsave(here("results", "markov_facet", 
              paste0("zambia_", "markov_facet", "_station_", stations[s], ".png")),
    ...
    gg <- ggplot(dat, aes_string(x = "s_doy_date", y = "probs", color = "product1", linetype = "lag_types")) +
      ...
      ggtitle(paste("Chance of rain:", products[p])) +
       theme(text = element_text(family = "Helvetica", size = 10),
          ...
          strip.background = element_rect(fill=NA))
  ggsave(here("results", "markov_facet", 
              paste0("zambia_", "_first_order_markov_facet_", products[p], ".png")),
         plot = gg, width = 6, height = 6)
}
```
    ...
    gg <- ggplot(dat, aes_string(x = "s_doy_date", y = "probs", color = "product1", linetype = "lag_types")) +
      ...
      ggtitle(paste("Chance of rain:", stations[s])) +
       theme(text = element_text(family = "Helvetica", size = 10),
          ...
          panel.grid = element_blank(),
          strip.background = element_rect(fill=NA))
  ggsave(here("results", "season_first_order_markov", 
              paste0("zambia_", "markov_season", "_station_", stations[s], ".png")),
         plot = gg, width = 6, height = 6)
}
```
    ...
    gg <- ggplot(dat, aes_string(x = "s_doy_date", y = "probs", color = "product1", linetype = "lag_types")) +
      ...
      ggtitle(paste("Chance of rain:", products[p])) + 
       theme(text = element_text(family = "Helvetica", size = 10),
          ...
          strip.background = element_rect(fill=NA))
  ggsave(here("results", "season_first_order_markov", 
              paste0("zambia_", "_first_order_markov_season_", products[p], ".png")),
         plot = gg, width = 6, height = 6)
}
```
