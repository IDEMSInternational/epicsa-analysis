...
files <- list.files("/media/johnbagiliko/TOSHIBA EXT/E-PICSA/data/satellite/CHIRPS", pattern = "chirps-v2", full.names = TRUE)
files <- head(files, -1)
...
files_path <- "/media/johnbagiliko/TOSHIBA EXT/E-PICSA/data/satellite/CHIRPS"
for(y in seq_along(years)) {
  ...
  f <- paste0(files_path, "/", "chirps-v2.0.",years[y], ".days_p05.nc")
  print(f)
  x <- tidync(f) %>%
...
saveRDS(total_means, here("results", "maps", "chirps_zambia_total.RDS"))
saveRDS(n_means, here("results", "maps", "chirps_zambia_n.RDS"))
saveRDS(mean_means, here("results", "maps", "chirps_zambia_mean.RDS"))

total_means <- readRDS(here("results", "maps", "chirps_zambia_total.RDS"))
n_means <- readRDS(here("results", "maps", "chirps_zambia_n.RDS"))
mean_means <- readRDS(here("results", "maps", "chirps_zambia_mean.RDS"))
...
chirps_total_rain <- ggplot(zambia_sf) + 
  ...
  labs(fill = "Total rainfall (mm)", title = "CHIRPS: Mean Yearly Rainfall (mm/year) 1981 - 2021") +
  scale_fill_viridis_c( direction = -1)
ggsave(here("results", "maps", "chirps_zambia_total_rain.png"), 
       chirps_total_rain,
       width = 16, height = 8)

chirps_n_rain <- ggplot(zambia_sf) + 
  geom_raster(data = n_means, aes(x = lon, y = lat, fill = n_rain)) +
  geom_sf(fill = NA, colour = "black") +
  geom_point(data = zambia_metadata, aes(x = longitude, y = latitude)) +
  geom_text_repel(data = zambia_metadata, aes(x = longitude, y = latitude, label = station)) +
  labs(fill = "Number of rain days", title = "CHIRPS: Mean Yearly Number of rain days (rainday/year)") +
  scale_fill_viridis_c(direction = -1)
ggsave(here("results", "maps", "chirps_zambia_n_rain.png"), 
       chirps_n_rain,
       width = 16, height = 8)

chirps_mean_rain <- ggplot(zambia_sf) + 
  geom_raster(data = mean_means, aes(x = lon, y = lat, fill = mean_rain)) +
  geom_sf(fill = NA, colour = "black") +
  geom_point(data = zambia_metadata, aes(x = longitude, y = latitude)) +
  geom_text_repel(data = zambia_metadata, aes(x = longitude, y = latitude, label = station)) +
  labs(fill = "Average rainfall (mm)", title = "CHIRPS: Mean Rain per rainday (mm/rainday)") +
  scale_fill_viridis_c(direction = -1, limits = c(5, 32))
ggsave(here("results", "maps", "chirps_zambia_mean_rain.png"), 
       chirps_mean_rain,
       width = 16, height = 8)
