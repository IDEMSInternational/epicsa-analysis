...
The results below focus on correct detection of dry days i.e. the proportion of dry days (from station) which are also estimated as dry from the product. A rain day threshold of 0.85 was used for both station and product data.
...
Detection is high for all products, with CHIRPS and TAMSAT having the highest detection, but these results are likely inflated by the inclusion of the larger part of the year where most days are dry (dry season).
...
correct_day: proportion of dry days which the product detects as dry i.e. dd/(dr + dd)
...
We see that the detection is much lower when considering only the period of the year were there is a reasonable chance of rain.
...
zmrain_drycount_novapr <- zmrain_long %>% 
  ...
  summarise(n = n(),
            ...
            dd = sum(!rainday & !prod_rainday)) %>%
  # proportion of dry days which the product detects as dry
  mutate(correct_dry_novapr = dd/(dr + dd),
         allyear_diff = 
