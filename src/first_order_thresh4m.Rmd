...
for (s in seq_along(stations)){
  ...
  predict_df_1$pr_rainday4_lag <- predict_df_1$rainday_lag
  predict_df_1$rainday_lag <- as.factor(predict_df_1$rainday_lag)
  predict_df_1$pr_rainday4_lag <- as.factor(predict_df_1$pr_rainday4_lag)
  ...
  for(i in seq_along(products)) {
    ...
    first_order_product4 <- glm(f_zero_order_product_4thres, data = dat_prod, family = binomial)
    ...
    predict_df_m[[paste0("station", "_", products[i])]] <- predict(first_order_station1, 
                                                                 ...
                                                                 type = "response")
    predict_df_m[[products[i]]] <- predict(first_order_product4, newdata = predict_df_m, 
                                           type = "response")
   ...
   predict_df_m$product <- products[[i]] 
  predict_df_m <- predict_df_m %>%
    mutate(product1 = ifelse(startsWith(variable, "station_"), "station_1mm", "product_4mm"))
...
for(s in seq_along(stations)) {
  ...
  ggsave(here("results", "season_first_order_markov", "thresh_4mm", 
              paste0("zambia_", "markov_season", "_station_4m", stations[s], ".png")),
    ...
    gg <- ggplot(dat, aes_string(x = "s_doy_date", y = "probs", color = "product1", linetype = "lag_types")) +
      ...
      ggtitle(paste("Chance of rain:", products[p])) + 
       theme(text = element_text(family = "Helvetica", size = 10),
          ...
          strip.background = element_rect(fill=NA))
  ggsave(here("results", "season_first_order_markov", "thresh_4mm", 
              paste0("zambia_", "_first_order_markov_season_4m", products[p], ".png")),
         plot = gg, width = 6, height = 6)
}
```
