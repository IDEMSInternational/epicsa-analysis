...
zambia_era5_moorings <- bind_rows(era5_dfs) %>%
  ...
  select(station, date, era5_rain = tp)

zambia_era5 <- readRDS(here("data", "satellite", "zambia_era5.RDS")) %>%
  rbind(zambia_era5_moorings)
