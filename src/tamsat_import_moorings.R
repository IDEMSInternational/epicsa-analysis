...
daily_rain <- readRDS(here("data", "station", "zambia_station.RDS")) #read the new metadata which contains Moorings
...
zambia_tamsat_moorings <- bind_rows(tamsat_dfs) %>% 
  select(station, date, tamsat_rain = rfe_filled)

zambia_tamsat <- readRDS("/media/johnbagiliko/TOSHIBA EXT/E-PICSA/data/station/cleaned/zambia_tamsat.RDS") %>%
  rbind(zambia_tamsat_moorings)

saveRDS(zambia_tamsat, "/media/johnbagiliko/TOSHIBA EXT/E-PICSA/data/station/cleaned/zambia_tamsat.RDS")
saveRDS(zambia_tamsat, here("data", "satellite", "zambia_tamsat.RDS"))
