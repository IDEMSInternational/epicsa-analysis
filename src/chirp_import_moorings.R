...
for(f in files) {
  for(i in seq_len(nrow(xy_extract))) {
    ...
    chirp_dfs[[length(chirp_dfs) + 1]] <- df
    count <- count + 1
    setTxtProgressBar(pb, count)
  }
}
zambia_moorings_chirp <- bind_rows(chirp_dfs) %>%
  mutate(date = as.Date(time, origin = as.Date("1980/1/1"))) %>% 
  select(station, date, chirp_rain = precip)

zambia_chirp <- readRDS(here("data", "satellite", "zambia_chirp.RDS")) %>%
  rbind(zambia_moorings_chirp)


saveRDS(zambia_chirp, here("data", "satellite", "zambia_chirp.RDS"))
