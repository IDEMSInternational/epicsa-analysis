...
zambia_moorings_chirps <- bind_rows(chirps_dfs) %>%
  mutate(date = as.Date(time, origin = as.Date("1980/1/1"))) %>% 
  select(station, date, chirps_rain = precip)

zambia_chirps <- readRDS(here("data", "satellite", "zambia_chirps.RDS")) %>%
  rbind(zambia_moorings_chirps)
