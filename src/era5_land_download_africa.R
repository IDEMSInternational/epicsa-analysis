...
# area: N/W/S/E
era5_land_download_area <- function(area, name, variable = "total_precipitation", dates) {
  if(missing(dates)) dates <- seq(as.Date("1950/01/01"), as.Date("2022/12/31"), by = "1 month")
  ...
  for(i in seq_along(dates)) {
    ...
    try(ncfile <- wf_request(user = user, request = request,
                             transfer = TRUE, path = "/media/johnbagiliko/TOSHIBA EXT/E-PICSA/data/satellite/ERA5_land",
                             time_out = 3 * 3600))
...
era5_land_download_area(area = c(38, -20, -35, 52), name = "africa", variable = "total_precipitation")

#check and download skipped files
files <- list.files("/media/johnbagiliko/TOSHIBA EXT/E-PICSA/data/satellite/ERA5_Land", pattern = "total_precipitation")
mm <- substr(files, 23, 28)
dates_done <- as.Date(paste(mm, 1), format = "%Y%m %d")
dates_all <- seq(as.Date("1950/01/1"), as.Date("2022/12/31"), by = "1 month")
dates_remain <- as.Date(dplyr::setdiff(dates_all, dates_done), origin = "1960-01-01")
while (length(dates_remain) > 0) {
  era5_land_download_area(area = c(38, -20, -35, 52), name = "africa", variable = "total_precipitation", dates = dates_remain)
  files <- list.files("/media/johnbagiliko/TOSHIBA EXT/E-PICSA/data/satellite/ERA5_land", pattern = "total_precipitation")
  mm <- substr(files, 23, 28)
  dates_done <- as.Date(paste(mm, 1), format = "%Y%m %d")
  dates_all <- seq(as.Date("1950/01/1"), as.Date("2022/12/31"), by = "1 month")
  dates_remain <- as.Date(dplyr::setdiff(dates_all, dates_done), origin = "1950-01-01")
}
