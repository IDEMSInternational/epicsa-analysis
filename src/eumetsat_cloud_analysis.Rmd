...
```{r}
  ...
  group_by(station, hour_8_to_7, Year_level, rain_level, s_doy) %>%
  summarise(mean_hourly_cloud = mean(CFC, na.rm = TRUE)) %>%
  filter(!is.na(rain_level))

  ggplot(moorings_mean_hour, aes(x = hour_8_to_7, y = mean_hourly_cloud, group = s_doy)) +
    geom_line() +
    xlab("Hour (8am-7am)") +
...
plot_cloud_cover <- function(df, yr){
  ...
  ggplot(moorings_year, aes(x = hour_8_to_7, y = CFC, group = s_doy)) +
    geom_line() +
    xlab("Hour (8am-7am)") +
...
for (yr in unique(merged_df$syear)){
